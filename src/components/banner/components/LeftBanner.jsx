...
import { motion } from "framer-motion";
import React from "react";
import { HiArrowRight } from "react-icons/hi2"; // Corrected if HiArrowRight was intended, or HiArrowRight from react-icons/hi
import { useTypewriter } from "react-simple-typewriter";
import { createRipple } from "../../layouts/RippleEffect"; // Assuming path is correct
import "../Banner.css"; // Assuming path is correct
import { AnimatedText, fadeUpProps } from "../Banner.utils.js"; // Assuming path is correct
import Media from "./Media"; // Assuming path is correct
...
const LeftBanner = () => {
  ...
  return (
    <motion.div
      {...fadeUpProps} // Assuming fadeUpProps is correctly defined in Banner.utils.js
      className="flex flex-col gap-8 md:gap-10 lg:gap-12 w-full md:w-11/12 mt-0 lg:mt-10 xl:mt-0"
    >
      <div className="flex flex-col gap-4 md:gap-5">
        <h4 className="tracking-[2px] mt-6 md:mt-0 text-sm sm:text-base font-normal font-titleFont w-fit h-fit text-pink-800 bg-pink-100 dark:text-pink-100 dark:bg-pink-800 rounded-full py-2 px-4 shadow-shadowTwo dark:shadow-shadowOne">
        ...
        <p className="text-gray-600 dark:text-white text-sm sm:text-base font-bodyFont leading-6 sm:leading-7 tracking-wide">
          I use animation as a third dimension by which to simplify and guide
          through every interaction. I’m not adding motion just to spruce things
          up, but doing it in ways that enhance the user experience.
        </p>
      </div>
      ...
      <div className="flex flex-col gap-16 lg:gap-20">
        <div className="flex items-center gap-4 flex-wrap sm:flex-nowrap">
          <button
            className="group hover:cursor-pointer" // Corrected cursor spelling if "cursopointer" was a typo
            onClick={() => scrollToSection("features")} // Assuming "features" is the correct ID for projects
            aria-label="View Projects"
          >
            <div
              className="flex items-center gap-2 w-fit h-fit bg-orange-100 text-amber-800 dark:bg-orange-800 dark:text-amber-100 font-medium px-4 py-3 rounded-full text-sm sm:text-base view-projects-button ripple-container elevatedShadow group-hover:shadow-none"
              onMouseDown={createRipple}
            >
              <div className="flex items-center gap-2 justify-center md:hidden">
                <span className="mb-0.5">View Projects </span>
          ...
          <button
            className="group hover:cursor-pointer" // Corrected cursor spelling
            onClick={() => scrollToSection("resume")}
            aria-label="View Resume"
          >
            <div
              className="flex items-center gap-2 w-fit h-fit bg-cyan-100 font-medium dark:bg-cyan-800 dark:text-blue-100 px-4 py-3 rounded-full text-blue-800 text-sm sm:text-base view-resume-button ripple-container elevatedShadow group-hover:shadow-none"
              onMouseDown={createRipple}
            >
              <div className="flex items-center gap-2 justify-center md:hidden">
                <span className="mb-0.5">View Resume </span>
