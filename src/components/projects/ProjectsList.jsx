...
const DelayedFallback = React.memo(() => {
  const [showLoader, setShowLoader] = useState(false);
  useEffect(() => {
    const timeout = setTimeout(() => setShowLoader(true), 300);
    return () => clearTimeout(timeout);
...
const ProjectsList = React.memo(function ProjectsList({
  ...
  const [isSubOpen, setSubOpen] = useState(false);
  const [hoveredSub, setHoveredSub] = useState(null);
  const defaultSubLabel = useMemo(
    () => (category === "ai" ? "All AI" : "All Stacks"),
    [category]
  );
  const [selectedSub, setSelectedSub] = useState(defaultSubLabel);
  const [currentPage, setCurrentPage] = useState(1);
  ...
  const displayedProjects = useMemo(
    () =>
      filteredProjects.slice(startIndexSlice, startIndexSlice + ITEMS_PER_PAGE),
    [filteredProjects, startIndexSlice]
  );
