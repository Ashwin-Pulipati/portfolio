...
export const FeedbackProvider = ({ children }) => {
  ...
  // Persist to localStorage whenever feedbackData changes
  useEffect(() => {
    localStorage.setItem("feedbackData", JSON.stringify(feedbackData));
  }, [feedbackData]);

  const getProjectFeedback = useCallback(
    (projectId) => {
      return (
  ...
  const setRating = useCallback(
    (projectId, newRating) => {
      setFeedbackData((prev) => {
        ...
        const avgRating =
          newRatingsCount > 0 ? newRatingSum / newRatingsCount : 0;
        return {
          ...prev,
          [projectId]: {
            ...current,
  ...
  const contextValue = useMemo(
    () => ({ getProjectFeedback, toggleLike, setRating, removeRating }),
    [getProjectFeedback, toggleLike, setRating, removeRating]
  );
