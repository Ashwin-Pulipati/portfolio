...
export const PrevButton = ({ emblaApi }) => {
  const [disabled, setDisabled] = useState(true);
  const scrollPrev = useCallback(() => emblaApi?.scrollPrev(), [emblaApi]);

  const update = useCallback((api) => setDisabled(!api.canScrollPrev()), []);
  useEffect(() => {
    if (!emblaApi) return;
    update(emblaApi);
...
export const NextButton = ({ emblaApi }) => {
  const [disabled, setDisabled] = useState(true);
  const scrollNext = useCallback(() => emblaApi?.scrollNext(), [emblaApi]);

  const update = useCallback((api) => setDisabled(!api.canScrollNext()), []);
  useEffect(() => {
    if (!emblaApi) return;
    update(emblaApi);
    emblaApi.on("select", update).on("reInit", update);
